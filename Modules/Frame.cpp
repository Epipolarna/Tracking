...
void Frame::drawObjects(Scalar color)
{
	for (std::vector<Object>::iterator it = objects.begin(); it != objects.end(); ++it)
	{
		...
		int thickness = 0.3;
		putText(image, objectText, Point(it->boundingBox.x-it->width, it->boundingBox.y+it->height+10), fontFace, fontScale, Scalar::all(255), thickness, 8);
	}
}
...
void Frame::drawObjects(std::vector<Object> & objects, Scalar color)
{
	for (std::vector<Object>::iterator it = objects.begin(); it != objects.end(); ++it)
	{
		...
		double fontScale = 0.3;
		int thickness = 0.3;
		putText(image, objectText, Point(it->boundingBox.x-it->width, it->boundingBox.y-10), fontFace, fontScale, Scalar::all(255), thickness, 8);
	}
}
